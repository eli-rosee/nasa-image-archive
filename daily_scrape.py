...
class DailyScrape:
    url_base = 'https://www.nasa.gov/image-article/'

    def __init__(self):
        self.https_response = requests.get(DailyScrape.url_base)
    ...
    def image_scrape(self):
        ...
        if self.https_response.status_code == 200:
            ...
            self.image_title = image_block.find('p', 'heading-22').text
            self.image_text = image_block.find('p', 'p-md').text
            self.image_src = image_block.find('img').attrs['src']
    ...
    # if the image src was successfully obtained, run the rest of program
        if self.image_src:
            ...
            r = requests.get(self.image_src, stream=True)
            ...
            with Image.open('images/daily_image.png') as image:
                ...
                for line in textwrap.wrap(self.image_text, width/15):
